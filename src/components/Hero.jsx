...
import {
    Code, Server, Database, Brain, Globe,
    Terminal, Bot, Cloud, Layout, Download
} from 'lucide-react';
...
const RotatingIcon = ({ icon: Icon, radius, duration, reverse = false, color }) => {
    return (
        <motion.div
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                >
                    ...
                    <h2 className="text-xl md:text-2xl text-gray-300 font-light mb-6 min-h-[40px]">
                        <Typewriter text="Automation Specialist | Full Stack Developer" speed={70} />
                    </h2>

                    <p className="text-gray-400 text-lg max-w-lg mb-10 leading-relaxed">
                        Results-driven developer passionate about crafting scalable web applications and AI-powered solutions. Transforming ideas into clean, efficient code.
                    </p>

                    <div className="flex flex-wrap gap-4">
                        <a href="#projects" className="px-8 py-4 rounded-full bg-primary-blue hover:bg-blue-600 text-white font-bold transition-all hover:scale-105 shadow-[0_0_20px_rgba(0,82,204,0.3)]">
                            View Projects
                        </a>
                        <a href="#about" className="px-8 py-4 rounded-full bg-transparent border border-brand-teal/50 text-brand-teal hover:bg-brand-teal/10 font-bold flex items-center gap-2 transition-all hover:scale-105">
                ...
                <div className="relative h-[600px] w-full flex items-center justify-center perspective-1000">
                    ...
                    <div className="relative z-20 w-32 h-32 rounded-full bg-gradient-to-br from-brand-blue to-brand-teal shadow-[0_0_50px_rgba(0,184,217,0.4)] flex items-center justify-center">
                        <span className="text-white font-black text-2xl tracking-widest">DEV</span>
                    </div>
                    ...
                    <RotatingIcon icon={Code} radius={140} duration={15} color="text-brand-blue" />
                    <RotatingIcon icon={Brain} radius={140} duration={15} reverse={true} color="text-brand-red" /> {/* Reverse logic needs positioning adjustment if on same orbit, usually we offset angle. Simplified here implies same start angle. */}
                    ...
                    <div className="absolute inset-0 flex items-center justify-center rotate-0">
                        <RotatingIcon icon={Code} radius={140} duration={20} color="text-blue-400" />
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center rotate-180">
                        <RotatingIcon icon={Layout} radius={140} duration={20} color="text-pink-400" />
                    </div>

                    {/* Middle Orbit (Radius ~210px) */}
                    <div className="absolute inset-0 flex items-center justify-center rotate-45">
                        <RotatingIcon icon={Server} radius={210} duration={25} reverse={true} color="text-green-400" />
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center rotate-135">
                        <RotatingIcon icon={Database} radius={210} duration={25} reverse={true} color="text-yellow-400" />
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center rotate-225">
                        <RotatingIcon icon={Globe} radius={210} duration={25} reverse={true} color="text-cyan-400" />
                    </div>

                    {/* Outer Orbit (Radius ~280px) */}
                    <div className="absolute inset-0 flex items-center justify-center rotate-90">
                        <RotatingIcon icon={Bot} radius={280} duration={30} color="text-purple-400" />
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center rotate-270">
                        <RotatingIcon icon={Cloud} radius={280} duration={30} color="text-orange-400" />
                    </div>
                    <div className="absolute inset-0 flex items-center justify-center rotate-0">
                        <RotatingIcon icon={Terminal} radius={280} duration={30} color="text-white" />
                    </div>
